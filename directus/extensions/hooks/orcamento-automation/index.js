...
export default ({ filter, action }, { services, exceptions, env, getSchema, logger }) => {
  ...
  action("items.update", async (meta, ctx) => {
    try {
      ...
      // Idempotência: só disparar se o payload do update inclui campo 'status'.
      // Previne reenvio quando PATCH é só view_count, resolution_note, etc.
      const updatePayload = meta.payload || {};
      if (!("status" in updatePayload)) return;
