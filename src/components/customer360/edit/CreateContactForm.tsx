...
export function CreateContactForm({ prefill }: CreateContactFormProps) {
  ...
  const canSubmit = form.company_name.trim().length > 0 && (form.phone.trim().length > 0 || form.mobile_phone.trim().length > 0 || form.email.trim().length > 0);
  ...
  const handleSaveAsLead = async () => {
    ...
    try {
      await createLead({
        ...
        contact_name: form.company_name.trim(),
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px] p-4">
        <div className="text-center space-y-2">
          ...
          <p className="text-sm font-medium text-foreground">Contacto criado com sucesso!</p>
          <p className="text-xs text-muted-foreground">A abrir ficha...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    /* Wrapper com scroll vertical próprio. Em landscape curto (e.g. 844x390),
       o form de 14 inputs + notas + 2 botões submit cabe acima do viewport
       (y=542 vs viewport=390 = 152px off-screen, Blocker #2 do F-MOBILE-VALIDATION).
       Solução: o form inteiro fica dentro de um scroll vertical que respeita
       a altura útil. Substitui o `min-h-[400px]` que forçava o conteúdo
       a esticar-se para baixo. */
    <div className="flex items-stretch justify-center p-2 sm:p-4" style={{ maxHeight: "calc(100dvh - var(--topbar-h, 0px))" }}>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <div>
                ...
                <Input value={form.company_name} onChange={(e) => handleChange("company_name", e.target.value)} placeholder="Nome da empresa ou pessoa" className="h-10 text-sm md:h-8" autoFocus />
              </div>
              <div>
                <Label className="text-xs text-muted-foreground">Pessoa de contacto</Label>
                <Input value={form.contact_person} onChange={(e) => handleChange("contact_person", e.target.value)} placeholder="Nome da pessoa" className="h-10 text-sm md:h-8" />
              </div>
            </div>
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <div>
                <Label className="text-xs text-muted-foreground">Telefone *</Label>
                <Input value={form.phone} onChange={(e) => handleChange("phone", e.target.value)} placeholder="Fixo" className="h-10 text-sm md:h-8" />
              </div>
              <div>
                <Label className="text-xs text-muted-foreground">Telemóvel</Label>
                <Input value={form.mobile_phone} onChange={(e) => handleChange("mobile_phone", e.target.value)} placeholder="9xx..." className="h-10 text-sm md:h-8" />
              </div>
            </div>
            <div>
              <Label className="text-xs text-muted-foreground">Email *</Label>
              <Input value={form.email} onChange={(e) => handleChange("email", e.target.value)} placeholder="email@empresa.pt" type="email" className="h-10 text-sm md:h-8" />
            </div>
            <p className="text-xs text-muted-foreground -mt-1">* Obrigatório: nome + (telefone, telemóvel ou email)</p>

            {/* Optional */}
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <div>
                <Label className="text-xs text-muted-foreground">Origem</Label>
                <Input value={form.source} onChange={(e) => handleChange("source", e.target.value)} placeholder="Ex: chamada, feira, site" className="h-10 text-sm md:h-8" />
              </div>
              <div>
                <Label className="text-xs text-muted-foreground">NIF</Label>
                <Input value={form.nif} onChange={(e) => handleChange("nif", e.target.value)} placeholder="123456789" className="h-10 text-sm md:h-8" />
              </div>
            </div>
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <div>
                <Label className="text-xs text-muted-foreground">Website</Label>
                <Input value={form.website} onChange={(e) => handleChange("website", e.target.value)} placeholder="www.empresa.pt" className="h-10 text-sm md:h-8" />
              </div>
              <div>
                <Label className="text-xs text-muted-foreground">Responsável</Label>
                <Input value={form.assigned_to} onChange={(e) => handleChange("assigned_to", e.target.value)} placeholder="Nome do comercial" className="h-10 text-sm md:h-8" />
              </div>
            </div>
            <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
              <div>
                <Label className="text-xs text-muted-foreground">Cidade</Label>
                <Input value={form.city} onChange={(e) => handleChange("city", e.target.value)} className="h-10 text-sm md:h-8" />
              </div>
              <div>
                <Label className="text-xs text-muted-foreground">Distrito</Label>
                <Input value={form.district} onChange={(e) => handleChange("district", e.target.value)} className="h-10 text-sm md:h-8" />
              </div>
              <div>
                <Label className="text-xs text-muted-foreground">Cód. Postal</Label>
                <Input value={form.postal_code} onChange={(e) => handleChange("postal_code", e.target.value)} placeholder="0000-000" className="h-10 text-sm md:h-8" />
              </div>
            </div>
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <div>
                <Label className="text-xs text-muted-foreground">Segmento</Label>
                <Input value={form.segment} onChange={(e) => handleChange("segment", e.target.value)} placeholder="Ex: Hotelaria" className="h-10 text-sm md:h-8" />
              </div>
              <div>
                <Label className="text-xs text-muted-foreground">Tipo de negócio</Label>
                <Input value={form.business_type} onChange={(e) => handleChange("business_type", e.target.value)} placeholder="Ex: Hotel, Restaurante" className="h-10 text-sm md:h-8" />
              </div>
            </div>
            ...
            <div className="flex gap-2">
              <Button onClick={handleCreate} disabled={saving || !canSubmit} className="flex-1 h-9 gap-2">
                {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <UserPlus className="h-4 w-4" />}
                {saving ? "A criar..." : "Criar contacto"}
              </Button>
              <Button variant="outline" onClick={handleSaveAsLead} disabled={saving || !form.company_name.trim()} className="h-9 gap-2">
                <UserRoundPlus className="h-4 w-4" />
                Guardar como Lead
              </Button>
            </div>

            {!canSubmit && form.company_name.trim() && (
              <p className="text-xs text-amber-600 text-center">Preenche telefone ou email para activar o botão "Criar contacto".</p>
            )}
          </div>
        </SectionCard>
      </div>
