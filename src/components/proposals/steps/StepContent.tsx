      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <Textarea
            ...
            placeholder="Contexto adicional que aparece antes dos produtos na proposta (opcional)..."
            rows={3}
          />
          <div className="flex items-center gap-2">
            <Button
          ...
          <p className="text-xs text-muted-foreground">
            Este texto aparece entre a saudação e os produtos na proposta enviada ao cliente.
          </p>
        </CardContent>
      </Card>
          ...
          <div className="relative">
            ...
            {showProductResults && (
              <div className="absolute z-50 top-full mt-1 w-full bg-popover border rounded-lg shadow-lg max-h-60 overflow-y-auto">
                {isSearching && <div className="p-3 text-sm text-muted-foreground">A procurar...</div>}
                {results.map((product) => (
                  <button
                    key={product.id}
                    type="button"
                    className="w-full text-left px-3 py-2 hover:bg-accent transition-colors border-b last:border-b-0 flex items-center gap-2"
                    onClick={() => addProduct(product)}
                  >
                    {(product.thumbnail || product.image_url || product.featured_media_url) && (
                      <img src={product.thumbnail || product.image_url || product.featured_media_url} alt="" className="w-8 h-8 rounded object-cover" />
                    )}
                    <div className="flex-1">
                      <div className="text-sm font-medium">{product.name || product.title}</div>
                      <div className="text-xs text-muted-foreground">{product.sku && `SKU: ${product.sku} · `}€{product.price?.toFixed(2)}</div>
                    </div>
