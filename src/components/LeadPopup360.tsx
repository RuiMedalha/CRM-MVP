...
export function LeadPopup360({
  ...
  const primary = useMemo(() => {
    return lead.display_name || lead.contact_phone || lead.email || "Lead";
  }, [lead.display_name, lead.contact_phone, lead.email]);
    ...
    (async () => {
      ...
      try {
        await markLeadMissedWithAggregation({
          ...lead,
          dedupe_key: lead.dedupe_key || computeDedupeKey({ phone: lead.contact_phone || undefined, email: lead.email }),
        });
  ...
  const handleAnswer = async () => {
    ...
    try {
      ...
      if (existing?.id) {
        ...
        onDismiss(lead.id);
        navigate(`/customer360-shell/${encodeURIComponent(String(existing.id))}`);
        return;
      }

      // Open the “card de contacto/lead” immediately to start filling
      const params = buildContactCreationUrl(lead, { includeLeadId: true, includeNif: true });
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-center gap-3 flex-1 min-w-0">
              ...
              <div className="flex-1 min-w-0">
                ...
                {lead.contact_phone && <p className="text-sm text-muted-foreground font-mono truncate">{lead.contact_phone}</p>}
