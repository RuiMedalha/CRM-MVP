...
import { isLeadEligibleForIncomingPopup } from "@/lib/leads-popup";
import { toast } from "@/hooks/use-toast";

/**
 * Polling-based listener for Directus `leads` collection.
 * - Só mostra popup para leads incoming recentes (VITE_LEADS_POPUP_MAX_AGE_SECONDS).
 * - Ignora central / type=call (Telecof → HubChat).
 * - Leads incoming antigos não disparam popup ao reabrir o CRM.
 */
export function useLeadListener360() {
  ...
  const pushLead = useCallback((lead: LeadItem) => {
    if (!isLeadEligibleForIncomingPopup(lead)) return;
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        ...
        if (!active) return;
        if (!lead || !isLeadEligibleForIncomingPopup(lead)) return;
        if (dismissedRef.current.has(lead.id)) return;
        if (incomingIdRef.current === lead.id) return;
